...
for trade in dn_item_trade_processed.find():
    ...
    buyer = trade['BUYER']
    seller = trade['SELLER']
    trade_3m = dn_itemtrade_3m_union.find_one({"ITEMSERIAL": item_serial})
    if(trade_3m is None):
        not_found += 1
    ...
    if buyer_char is None or seller_char is None:
        ...
        dn_item_trade_processed.update_one({"_id": trade["_id"]}, {"$set":{"characterid_buyer": buyer_char, 'characterid_seller': seller_char}})
        processed+= 1
